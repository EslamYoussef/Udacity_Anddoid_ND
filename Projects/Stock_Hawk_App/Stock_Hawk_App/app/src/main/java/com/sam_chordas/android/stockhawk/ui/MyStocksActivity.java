...
public class MyStocksActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            mServiceIntent.putExtra(Keys.TAG, Keys.INIT);
...
//Get Expanded position, in case of selected position in Widget
        if (null != getIntent() && null != getIntent().getExtras()) {
            if (getIntent().getExtras().containsKey(Keys.SELECTED_POSITION)) {
                mExpandedPosition = getIntent().getExtras().getInt(Keys.SELECTED_POSITION);
            }
        }
                            ...
                            .input(R.string.input_hint, R.string.input_prefill, new MaterialDialog.InputCallback() {
                                ...
                                public void onInput(MaterialDialog dialog, CharSequence input) {
                                    ...
                                    if (c.getCount() != 0) {
                                        ...
                                        // Add the stock to DB
                                        mServiceIntent.putExtra(Keys.TAG, Keys.ADD);
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if (data.getCount() == 0) {
            if (!Utils.isConnected(this)) {
                tvEmptyView.setText(R.string.no_network);
            }
